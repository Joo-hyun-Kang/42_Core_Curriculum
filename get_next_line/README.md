...
### 에러를 방어하는 곳은 경계인 read()함수를 사용하는 try_enqueue_fd 처리
* 구현 시에 모든 부분에서 에러처리를 한다는 것은 코드의 가독성을 해치고 디버깅도 힘들기에 문제가 생길 수 있는 경계인 try_enqueue_fd에서 막는다.
...
* 파일 디스크립터가 몇개까지 들어올 지는 시스템마다 다르다. 검색결과 설정에 따라 512, 1024, 2048 등 크기가 어려개로 존재한다. 
