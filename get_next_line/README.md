...
## 이슈 : read함수 EOF까지 읽기 
* https://vallhalla-edition.tistory.com/42
* 블록되는 이슈
...
* 버퍼가 1로 읽어올 때 마지막에 리턴이 0을 반환해주지만 덩어리로 버퍼가 1이상이면 그 나머지 값을 다시 처음으로 돌아가서 읽어버림 그래서 ret == 0은 버퍼 사이즈가 1일 때하는 예외처리
* 아 바로 위에거 잘 못 적음
그냥 그래도 BUFFER_SIZE보다 작으면 되니까 어쩄뜬 1이라도 0 반환하니까 ret < BUFFER_SIZE으로 가능


##이슈 : 버퍼 사이즈가 1 && EOF일 때 널 반환 vs \0반환

\n 다음에 아무것도 없을 때 -> Null 반환
|1
|
...
버퍼사이즈 1일 때 마지막 줄 -> '\0'문자 붙어주고 반환
|1

4개씩 읽을 대 -> '\0'문자 붙여주고 반환
|123
