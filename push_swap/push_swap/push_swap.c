...
void ft_split_a_to_b(push_swap_member_t *member, ft_stack_t *pivots, ft_stack_t *queue, int stack_a_count)
{
	...
	pb_count = 0;
	rb_count = 0;
	two_thirds_pivot = ft_pop_front(pivots);
	one_thirds_pivot = ft_pop_front(pivots);
	while (i < stack_a_count)
	{
		top = ft_peak(member->a);
		if (top >= two_thirds_pivot)
		{
	...
	{
		ft_rrr(member->a, member->b, queue);
		i++;
	}
	while (i < ra_count)
	{
	...
	ft_split_b_to_a(member, pivots, queue, pb_count - rb_count);
}
...
void ft_split_b_to_a(push_swap_member_t *member, ft_stack_t *pivots, ft_stack_t *queue, int count)
{
	...
	ft_split_a_to_b(member, pivots, queue, pa_count - ra_count);
	i = 0;
	while (i < ra_count && i < rb_count)
	{
		ft_rrr(member->a, member->b, queue);
...
void ft_put_queue(ft_stack_t *queue)
{
	...
	while (i < queue->size / 2)
	{
		...
		temp = temp->next;
		i++;
		if ((res1 == COMMAND_RA && res2 == COMMAND_RB) || (res1 == COMMAND_RB && res2 == COMMAND_RA))
			ft_put_command(COMMAND_RR);
		else if ((res1 == COMMAND_RRA && res2 == COMMAND_RRB) || (res1 == COMMAND_RRB && res2 == COMMAND_RRA))
			ft_put_command(COMMAND_RRR);
		else if ((res1 == COMMAND_RR && res2 == COMMAND_RRR) || (res1 == COMMAND_RRR && res2 == COMMAND_RR))
			continue;
		else if ((res1 == COMMAND_PA && res2 == COMMAND_PB) || (res1 == COMMAND_PB && res2 == COMMAND_PA))
			continue;
		else if ((res1 == COMMAND_RA && res2 == COMMAND_RRA) || (res1 == COMMAND_RB && res2 == COMMAND_RRB))
			continue;
		else if ((res1 == COMMAND_SA && res2 == COMMAND_SA) || (res1 == COMMAND_SB && res2 == COMMAND_SB))
			continue;
		else
		{
			ft_put_command(res1);
